#!/usr/bin/env python3
"""
Script para descubrir categorías de Día %
Similar a test_lareina_mapeo_categorias.py y test_lagallega_mapeo_completo.py

Día usa API similar a VTEX - probablemente tiene endpoints de categoría
"""
...
def probar_endpoint_categorias():
    """Intenta obtener árbol de categorías de Día"""
    
    print("=" * 80)
    print("🔍 BUSCANDO ENDPOINT DE CATEGORÍAS EN DÍA %")
    print("=" * 80)
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    with open('dia_categorias_response.json', 'w', encoding='utf-8') as f:
                        json.dump(data, f, indent=2, ensure_ascii=False)
                    print(f"   💾 Guardado en dia_categorias_response.json")
...
def analizar_estructura_producto():
    """Analiza un producto de Día para ver si tiene info de categoría"""
    ...
    try:
        ...
        if response.status_code in [200, 206]:
            ...
            if data and len(data) > 0:
                ...
                with open('dia_producto_ejemplo.json', 'w', encoding='utf-8') as f:
                    json.dump(producto, f, indent=2, ensure_ascii=False)
                print(f"\n💾 Producto completo guardado en dia_producto_ejemplo.json")
...
if __name__ == "__main__":
    print("\n")
    print("╔════════════════════════════════════════════════════════════════════════════╗")
    print("║                 MAPEO DE CATEGORÍAS - DÍA %                                ║")
    print("║                                                                            ║")
    print("║  Objetivo: Descubrir estructura de categorías en API de Día               ║")
    ...
    print("   - dia_categorias_response.json (si se encontró endpoint)")
    print("   - dia_producto_ejemplo.json (estructura de producto)")
    print("\n")
